    ...
    : control(c), lexer(token_stream, location_table, line_table, control)
{
    ...
    m_last_nameAST_seen = NULL;
    m_doing_int_assignment = false;
    m_current_int_value = 0;
}
...
bool Parser::parseTranslationUnit(TranslationUnitAST *&node)
{
    ...
    while (token_stream.lookAhead()) {
        ...
        int startDecl = token_stream.cursor();
...
bool Parser::parseOperatorFunctionId(AST *&node)
{
    ...
    if (parseOperator(op)) {
        ...
        TypeSpecifierAST *spec = 0;
	RangeNumber range;
        if (!parseSimpleTypeSpecifier(spec, range)) {
            syntaxError();
            return false;
        }
        spec->setCvQualify(cv);
...
bool Parser::parseSimpleTypeSpecifier(TypeSpecifierAST *&node, RangeNumber &range, bool onlyIntegral)
{
    std::size_t start = token_stream.cursor();
    ...
    if (isIntegral) {
        ...
        for (int i=ast->startToken(); i<ast->endToken(); ++i) 
         fprintf(stderr, "ast = %s\n", token_stream.tokenText(i).c_str()); 
        for (int i=name->startToken(); i<name->endToken(); ++i) 
...
bool Parser::parseTemplateArgument(AST *&node)
{
    ...
    AbstractExpressionAST *expr = 0;
    RangeNumber range2;
    if (!parseLogicalOrExpression(expr, range2, true)) {
        return false;
    }
    node = expr;
...
bool Parser::parseTypeSpecifier(TypeSpecifierAST *&spec)
{
    ...
    RangeNumber range;
    if (parseElaboratedTypeSpecifier(ast) || parseSimpleTypeSpecifier(ast, range)) {
        spec = ast;
        spec->setCvQualify(cv);
...
bool Parser::parseDeclarator(DeclaratorAST *&node)
{
    ...
    if (token_stream.lookAhead() == '(') {
        ...
        if (token_stream.lookAhead() == ':') {
            token_stream.nextToken();
            AbstractExpressionAST *expr = 0;
            if (!parseConstantExpression(expr, range)) {
...

    {
        ...
        while (token_stream.lookAhead() == '[') {
            ...
            token_stream.nextToken();
            AbstractExpressionAST *expr = 0;
            parseCommaExpression(expr, range);
...
bool Parser::parseInitializerClause(AST *&node)
{
    if (token_stream.lookAhead() == '{') {
        ...
        AbstractExpressionAST *expr = 0;
	RangeNumber range;
        if (!parseAssignmentExpression(expr, range)) {
            //reportError(("Expression expected"));
        }
        node = expr;
    }
...
bool Parser::parseCondition(ConditionAST *&node, bool initRequired)
{
    ...
    token_stream.rewind(start);
    AbstractExpressionAST *expr = 0;
    if (!parseCommaExpression(expr, range)) {
        return false;
    }
...
bool Parser::parseCompoundStatement(StatementAST *&node)
{
    ...
    while (token_stream.lookAhead()) {
        ...
        StatementAST *stmt = 0;
        int startStmt = token_stream.cursor();
...
bool Parser::parseLabeledStatement(StatementAST *&node)
{
	...
	RangeNumber range;
        if (!parseConstantExpression(expr, range)) {
            ...
            if (!parseConstantExpression(expr2, range)) {
                reportError(("expression expected"));
            }
        }
        ADVANCE(':', ":");
...
bool Parser::parsePrimaryExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    std::size_t start = token_stream.cursor();
    ...
    switch(token_stream.lookAhead()) {
	    ...
	    {
	       int value =token_stream.tokenAsInt( token_stream.cursor() ) ;
	       range = RangeNumber( value, value );
	       fprintf(stderr, "Found an int '%d'", value);
	    }
...
*/
bool Parser::parsePostfixExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    std::size_t start = token_stream.cursor();
    ...
    if (parseName(name, true)) {
        ...
        if (has_template_args && token_stream.lookAhead() == '(') {
            ...
            if (parseCastExpression(cast_expr, range) && cast_expr->nodeType() == NodeType_CastExpression) {
                token_stream.rewind(saved_pos);
                parsePrimaryExpression(expr, range);
                goto L_no_rewind;
            }
        }
    }
...
L_no_rewind:
    if (!expr && parseSimpleTypeSpecifier(typeSpec, range) && token_stream.lookAhead() == '(') {
        token_stream.nextToken(); // skip '('
        parseCommaExpression(expr, range);
        CHECK(')', ")");
    } else if (expr) {
        ...
        if (!parsePrimaryExpression(expr, range))
            return false;
    }
...
bool Parser::parseUnaryExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    std::size_t start = token_stream.cursor();
    ...
    switch(token_stream.lookAhead()) {
        ...
        case '~':
        {
            ...
            token_stream.nextToken();
            AbstractExpressionAST *expr = 0;
            if (!parseCastExpression(expr, range))
                return false;
        ...
        case Token_sizeof:
        {
            ...
            AbstractExpressionAST *expr = 0;
            if (!parseUnaryExpression(expr, range))
                return false;
    ...
    return parsePostfixExpression(node, range);
}
...
bool Parser::parseCastExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    std::size_t start = token_stream.cursor();
    ...
    token_stream.rewind(start);
    return parseUnaryExpression(node, range);
}

bool Parser::parsePmExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parseCastExpression(ast, range) || !ast) // ### fixme
        return false;
	...
	RangeNumber range2; //Am I even going to go into the pain of parsing ptrs on the fly ?  JOHNFLUX FIXME
        if (!parseCastExpression(rightExpr, range2))
            return false;
...
bool Parser::parseMultiplicativeExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parsePmExpression(ast, range))
        return false;
    
    char nextop = token_stream.lookAhead();
    while (nextop == '*' || nextop == '/' || nextop == '%') {
        int startOp = token_stream.cursor();
        AST_FROM_TOKEN(op, startOp);
        token_stream.nextToken();
        ...
        AbstractExpressionAST *rightExpr = 0;
	RangeNumber range2;
        if (!parsePmExpression(rightExpr, range2))
            return false;
        if(nextop == '*') 
	    range = range * range2;
	else if(nextop == '/')
...
bool Parser::parseAdditiveExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parseMultiplicativeExpression(ast, range))
        return false;
    char nextop = token_stream.lookAhead();
    while (nextop == '+' || nextop == '-') {
        int startOp = token_stream.cursor();
        ...
        AbstractExpressionAST *rightExpr = 0;
	RangeNumber range2;
        if (!parseMultiplicativeExpression(rightExpr, range2))
            return false;
...
bool Parser::parseShiftExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parseAdditiveExpression(ast, range))
        return false;
    ...
    while (token_stream.lookAhead() == Token_shift) {
        ...
        AbstractExpressionAST *rightExpr = 0;
	RangeNumber range2;
        if (!parseAdditiveExpression(rightExpr, range2))
            return false;
	//JOHNFLUX FIXME Do the actual shift.  We don't seem to know which direction the stupid shift is in
...
bool Parser::parseRelationalExpression(AbstractExpressionAST *&node, RangeNumber &range, bool templArgs)
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parseShiftExpression(ast, range))
        return false;
    ...
    while (token_stream.lookAhead() == '<' || (token_stream.lookAhead() == '>' && !templArgs) ||
        ...
        AbstractExpressionAST *rightExpr = 0;
	RangeNumber range2;
        if (!parseShiftExpression(rightExpr, range2))
            return false;
	//FIXME - use value2
...
bool Parser::parseEqualityExpression(AbstractExpressionAST *&node, RangeNumber &range, bool templArgs)
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parseRelationalExpression(ast, range, templArgs))
        return false;
    ...
    while (token_stream.lookAhead() == Token_eq || token_stream.lookAhead() == Token_not_eq) {
        ...
        AbstractExpressionAST *rightExpr = 0;
	RangeNumber range2;
        if (!parseRelationalExpression(rightExpr, range2, templArgs))
            return false;
	//FIXME - use value2
        BinaryExpressionAST *tmp = CreateNode<BinaryExpressionAST>(m_pool);
        tmp->setOp(op);
...
bool Parser::parseAndExpression(AbstractExpressionAST *&node, RangeNumber &range, bool templArgs)
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parseEqualityExpression(ast, range, templArgs))
        return false;
    ...
    while (token_stream.lookAhead() == '&') {
        ...
        AbstractExpressionAST *rightExpr = 0;
	RangeNumber range2;
        if (!parseEqualityExpression(rightExpr, range2, templArgs))
            return false;
...
bool Parser::parseExclusiveOrExpression(AbstractExpressionAST *&node, RangeNumber &range, bool templArgs)
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parseAndExpression(ast, range, templArgs))
        return false;
    ...
    while (token_stream.lookAhead() == '^') {
        ...
        AbstractExpressionAST *rightExpr = 0;
	RangeNumber range2;
        if (!parseAndExpression(rightExpr, range2, templArgs))
            return false;
...
bool Parser::parseInclusiveOrExpression(AbstractExpressionAST *&node, RangeNumber &range, bool templArgs)
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parseExclusiveOrExpression(ast, range, templArgs))
        return false;
    ...
    while (token_stream.lookAhead() == '|') {
        ...
        AbstractExpressionAST *rightExpr = 0;
	RangeNumber range2;
        if (!parseExclusiveOrExpression(rightExpr, range2, templArgs))
            return false;
...
bool Parser::parseLogicalAndExpression(AbstractExpressionAST *&node, RangeNumber &range, bool templArgs)
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parseInclusiveOrExpression(ast, range, templArgs))
        return false;
    ...
    while (token_stream.lookAhead() == Token_and) {
        ...
        AbstractExpressionAST *rightExpr = 0;
	RangeNumber range2;
        if (!parseInclusiveOrExpression(rightExpr, range2, templArgs))
            return false;
...
bool Parser::parseLogicalOrExpression(AbstractExpressionAST *&node, RangeNumber &range, bool templArgs)
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parseLogicalAndExpression(ast, range, templArgs))
        return false;
    ...
    while (token_stream.lookAhead() == Token_or) {
        ...
        AbstractExpressionAST *rightExpr = 0;
	RangeNumber range2;
        if (!parseLogicalAndExpression(rightExpr, range2, templArgs))
            return false;
...
bool Parser::parseConditionalExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    std::size_t start = token_stream.cursor();
    //JOHNFLUX - FIXME determine from value whether to parse left or right
    AbstractExpressionAST *ast = 0;
    if (!parseLogicalOrExpression(ast, range))
        return false;
	...
	RangeNumber range2;
        if (!parseExpression(leftExpr, range2))
            return false;
        ...
        AbstractExpressionAST *rightExpr = 0;
	RangeNumber range3;
        if (!parseAssignmentExpression(rightExpr, range3))
            return false;
	...
	range = range.doif(range2, range3);
    }
...
bool Parser::parseAssignmentExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    std::size_t start = token_stream.cursor();
    ...
    if (token_stream.lookAhead() == Token_throw && !parseThrowExpression(ast))
        return false;
    else if (!parseConditionalExpression(ast, range))
        return false;
    ...
    while (token_stream.lookAhead() == Token_assign || token_stream.lookAhead() == '=') {
        ...
        AbstractExpressionAST *rightExpr = 0;
        if (!parseConditionalExpression(rightExpr, range))
            return false;
	
	printf("Assignment of %s to ", token_stream.tokenText(ast->startToken()).c_str());
	range.display();
	printf("\n");
...
bool Parser::parseConstantExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    return parseConditionalExpression(node, range);
}

bool Parser::parseExpression(AbstractExpressionAST *&node, RangeNumber &range)
{
    return parseCommaExpression(node, range);
}

bool Parser::parseCommaExpression(AbstractExpressionAST *&node, RangeNumber &range )
{
    std::size_t start = token_stream.cursor();

    AbstractExpressionAST *ast = 0;
    if (!parseAssignmentExpression(ast, range))
        return false;
    ...
    while (token_stream.lookAhead() == ',') {
        ...
        AbstractExpressionAST *rightExpr = 0;
        if (!parseAssignmentExpression(rightExpr, range))
            return false;
