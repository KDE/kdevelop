...
Lnaguage support can be written either as a source tree build or as a
...
new class wizard: (See adasupport, phpsupport, cppsupport or javasupport)
    virtual void addClass();
add method dialog: (See cppsupport or javasupport)
    virtual void addMethod(const QString &className);
add attribute dialog: (See cppsupport or javasupport)
...
parts/cppsupport for an example of using catalog. parts/pascalsupport is
completely based on a catalog (which is filled by a pascal parser - check
pascal.tree.g grammar file). Catalog is stored on disk in the database
file (Berkeley db) If you use catalog, your class browser will be parts/classbrowser.
...
c-based languages) you can extend astyle library (lib/astyle) that is used
