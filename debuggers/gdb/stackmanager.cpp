...
int StackManager::rowCount(const QModelIndex & parent) const
{
    ...
    if (!parent.internalPointer()) {
        ...
        ThreadItem* thisThread = m_threads.at(parent.row());
        // Fetch more frames if none have previously been fetched
        if (!thisThread->hasFetchedMoreFrames())
            thisThread->fetchMoreFrames();
        
        return thisThread->frames().count();
    }
...
void StackManager::prepareRemoveFrames(ThreadItem* thread, int index, int endIndex)
{
    beginRemoveRows(indexForThread(thread), index, endIndex);
}

void StackManager::completeRemoveFrames()
{
    endRemoveRows();
}
