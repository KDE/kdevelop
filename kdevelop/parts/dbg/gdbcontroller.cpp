...
void GDBController::programNoApp(const QString& msg, bool msgBox)
{
  ...
  if (msgBox)
    KMessageBox::sorry (0, i18n("Error"), i18n("gdb message:\n")+msg);
}
...
//        actOnProgramPause(QString());
    ...
    case START_Core:
    {
      ...
      if (badCore_ && strncmp(buf, "Core was generated by", 21) == 0)
        KMessageBox::sorry( 0, i18n("Mismatched core file"),
                          i18n("gdb message:\n")+badCore_ + "\n" + QString(buf)+"\n\n"+
...
void GDBController::parseProgramLocation(char* buf)
{
  ...
  if ((start = regExp3.match(buf, 0)) >= 0)
    emit showStepInSource(QCString(), -1, QCString(buf,
                              (strchr(buf, ' ')-buf)+1));
  else
...
char* GDBController::parse(char* buf)
{
  ...
  {
    char* parsed;
    if ((GDBCmd)*unparsed == BLOCK_START)
...
void GDBController::slotStart(const QString& application, const QString& args, const QString &sDbgShell)
{
  ...
  if (tty.isEmpty())
  {
    KMessageBox::sorry(0, i18n("Error"), i18n("gdb cannot use the tty* or pty* devices\n"
