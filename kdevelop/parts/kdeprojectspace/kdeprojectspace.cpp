...
void KDEProjectSpace::updateAdminFiles(){
  cerr << "\nkdevelop (KDE ProjectSpace) enter KDEProjectSpace::updateAdminFiles()";
  ...
  QFile file(m_path + "/configure.in.in");
    ...
    if ( file.open(IO_WriteOnly) ){
      ...
      tw << "dnl KDE ProjectSpace plugin (c) 2000 by KDevelop Team\n";
      tw << "dnl\n";
...
QStringList KDEProjectSpace::allDirectories(){
  QList<Project>* pProjects = allProjects();
  Project* pProject=0;
  QStringList dirs;
  QFileInfo info;
