...
KDEObjcProjectSpace::KDEObjcProjectSpace(QObject* parent,const char* name)
		: AutomakeProjectSpace(parent,name)
{
	kdDebug(9020) << "KDEObjcProjectSpace created" << endl;
	
	setInstance(KDEObjcProjectSpaceFactory::instance());

	KStandardDirs* std_dirs = KGlobal::dirs();
	

	// projectspace
	m_projectspaceTemplate = std_dirs->findResource("data","kdevelop/projectspaces/kdeobjc_projectspace.tar.gz");
	m_language = "C++";
	m_pAboutData=0;


	setXMLFile("kdevkdeobjcprojectspaceui.rc");
}

KDEObjcProjectSpace::~KDEObjcProjectSpace(){
}

KDevMakeFrontend* KDEObjcProjectSpace::makeFrontend(){
	return static_cast<KDevMakeFrontend*>(componentManager()->component("KDevMakeFrontend"));
}

void KDEObjcProjectSpace::setupGUI(){

  AutomakeProjectSpace::setupGUI(); // set actions for "set active project"
  kdDebug(9020) << "Building KDEObjcProjectSpace GUI" << endl;
  KAction *pAction;
...
void KDEObjcProjectSpace::slotProjectAddNewTranslationFile(){
    kdDebug(9020) << "Add new &Translation File (KDE)... actived" << endl;
}


void KDEObjcProjectSpace::updateAdminFiles(){
  cerr << "\nkdevelop (KDE Objective-C ProjectSpace) enter KDEProjectSpace::updateAdminFiles()";
  ...
  QFile file(m_path + "/configure.in.in");
    ...
    if ( file.open(IO_WriteOnly) ){
      ...
      tw << "dnl KDE Objective-C ProjectSpace plugin (c) 2000 by KDevelop Team\n";
      tw << "dnl\n";
...
QStringList KDEObjcProjectSpace::allDirectories(){
  QList<Project>* pProjects = allProjects();
  Project* pProject=0;
  QStringList dirs;
  QFileInfo info;
...
bool KDEObjcProjectSpace::writeGlobalConfig(QDomDocument& doc,QDomElement& psElement){
  AutomakeProjectSpace::writeGlobalConfig(doc,psElement);
  QDomElement kdeElement = psElement.appendChild(doc.createElement("KDEObjcProjectSpace")).toElement();
  return true;
}
bool KDEObjcProjectSpace::writeUserConfig(QDomDocument& doc,QDomElement& psElement){
  AutomakeProjectSpace::writeUserConfig(doc,psElement);
  QDomElement kdeElement = psElement.appendChild(doc.createElement("KDEObjcProjectSpace")).toElement();
  return true;
}

bool KDEObjcProjectSpace::readGlobalConfig(QDomDocument& doc,QDomElement& psElement){
  AutomakeProjectSpace::readGlobalConfig(doc,psElement);
  QDomElement kdeElement = psElement.namedItem("KDEObjcProjectSpace").toElement();
  if(kdeElement.isNull()){
    kdDebug(9020) << "KDEObjcProjectSpace::readGlobalConfig not \"KDEObjcProjectSpace\" tag found!" << endl;
    return false;
  }
  else{
    kdDebug(9020) << "KDEObjcProjectSpace::readGlobalConfig  \"KDEObjcProjectSpace\" tag found!" << endl;
  }
  return true;
}


bool KDEObjcProjectSpace::readUserConfig(QDomDocument& doc,QDomElement& psElement){
  AutomakeProjectSpace::readUserConfig(doc,psElement);
  QDomElement kdeElement = psElement.namedItem("KDEObjcProjectSpace").toElement();
  if(kdeElement.isNull()){
    kdDebug(9020) << "KDEObjcProjectSpace::readUserConfig not \"KDEObjcProjectSpace\" tag found!" << endl;
    return false;
  }
  else{
    kdDebug(9020) << "KDEObjcProjectSpace::readUserConfig  \"KDEObjcProjectSpace\" tag found!" << endl;
  }
  return true;
}


KAboutData* KDEObjcProjectSpace::aboutPlugin(){
  if (m_pAboutData == 0){
    m_pAboutData= new KAboutData( "KDE Objective-C", I18N_NOOP("KDE Objective-C ProjectSpace"),
				  "0.1", "kk",
...
QList<FileGroup> KDEObjcProjectSpace::defaultFileGroups(){
  QList<FileGroup> list;
  FileGroup* pGroup = new FileGroup();
  pGroup->setName(i18n("Source Files"));
  pGroup->setFilter("*.cpp;*.c;*.cc;*.cxx;*.C;*.m");
  list.append(pGroup);
...
void KDEObjcProjectSpace::updateMakefilesAm(){
}
void KDEObjcProjectSpace::slotBuildMake(){
  updateAdminFiles();
    makeFrontend()->executeMakeCommand("make");
}
void KDEObjcProjectSpace::slotBuildRebuildAll(){
}
void KDEObjcProjectSpace::slotBuildCleanRebuildAll(){
    makeFrontend()->executeMakeCommand("make clean");
}
void KDEObjcProjectSpace::slotBuildExecute(){
}
void KDEObjcProjectSpace::slotBuildExecuteWithArgs(){
}
void KDEObjcProjectSpace::slotBuildDistClean(){
    makeFrontend()->executeMakeCommand("make distclean");
}
void KDEObjcProjectSpace::slotBuildAutoconf(){
}
void KDEObjcProjectSpace::slotBuildConfigure(){
    makeFrontend()->executeMakeCommand("./configure");
}

#include "kdeobjcprojectspace.moc"
