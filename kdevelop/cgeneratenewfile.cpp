...
QString  CGenerateNewFile::genGNUFile(QString abs_name,CProject* prj){
  QString name  = QFileInfo(abs_name).fileName();
  QString template_file = KApplication::kde_datadir() + 
    "/kdevelop/templates/"+ name + "_template";
  QString author = prj->getAuthor();
  QString email = prj->getEmail();
  QStrList list;
  QString str;
  ...
  QTextStream stream(&file);
  if(file.open(IO_ReadOnly)){ // read the templatefile
    while(!stream.eof()){
      list.append(stream.readLine());
    }
  }
