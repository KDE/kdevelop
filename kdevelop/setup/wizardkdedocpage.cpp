...
//!? conflict with KDE-1.x            << "/opt/kde/share/doc/HTML/en/kdelibs"     // normal dist
  ...
  {
    #if KDE_QTVER >= 3
    if( QFileInfo(m_pInstallState->kde+"/kdecore/kdecore.tag.gz").exists()
    	|| QFileInfo(m_pInstallState->kde+"/kdecore/kdecore.tag").exists() )
    #else
  ...
  {
    #if KDE_QTVER >= 3
    if( QFileInfo((*it)+"/kdecore/kdecore.tag.gz").exists()
      || QFileInfo((*it)+"/kdecore/kdecore.tag").exists() )
    #else
  ...
  if (kde_docs_found) {
    ...
    label = new QLabel(i18n("The documentation of the KDE-library could not be found.\n\n"
                            #if KDE_QTVER >= 3
                            "Please make sure you have KDELibs-Doc installed and set the directory manually."), m_vbox);
                            #else
