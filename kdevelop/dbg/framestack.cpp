...
void FrameStack::slotSelectionChanged(QListViewItem *thisItem)
{
...
// someone wants us to select this frame, possibly on a thread. We may have
// the data but we still need to tell others that we have been changed.
// and if we don't have the data then gdb needs to suplly it.
void FrameStack::slotSelectFrame(int frameNo, int threadNo)
{
...
QListViewItem* FrameStack::lastChild() const
{
...
void FrameStack::parseGDBThreadList(char* str)
{
...
QCString FrameStack::getFrameParams(int frameNo, int threadNo)
{
...
QString FrameStack::getFrameName(int frameNo, int threadNo)
{
  if (FrameStackItem* frame = findFrame(frameNo, threadNo))
  {
    QString frameStr = frame->text(0);
    ...
    if (char* paramStart = strchr(frameData, '('))
    {
      ...
      if (threadNo != -1)
      {
        ...
        return frameName.arg(threadNo).arg(frameNo).arg(QCString(fnstart, paramStart-fnstart+1));
      }
...
ThreadStackItem* FrameStack::findThread(int threadNo)
{
...
FrameStackItem* FrameStack::findFrame(int frameNo, int threadNo)
{
...
FrameStackItem::FrameStackItem(FrameStack* parent, const QString& frameDesc) :
...
{
  if (open)
    ((FrameStack*)listView())->slotSelectFrame(0, threadNo());
...
ThreadStackItem::ThreadStackItem(FrameStack* parent, const QString& threadDesc) :
