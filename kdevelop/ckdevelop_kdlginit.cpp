...
void CKDevelop::initKDlg()
{
  ...
  kdlg_tabctl= new CTabCtl(topSplitter);
...
//  kdlgTopSplitter = new QSplitter(Qt::Vertical, topSplitter,"kdlgTopSplitter");
// panner works the other way
  kdlgTopSplitter = new QSplitter(topSplitter,"kdlgTopSplitter");

  kdlg_prop_widget = new KDlgPropWidget(this,kdlgTopSplitter,"KDlg_properties_widget"); // the properties window of kdlg
  kdlg_edit_widget = new KDlgEditWidget(this,kdlgTopSplitter,"KDlg_edit_widget"); // the editing view of kdlg

#warning FIXME QSplitter missing functions???
//  kdlgTopSplitter->showLabels(true);
//  kdlgTopSplitter->setLabels(i18n("Widget Editor"),i18n("Widget Properties"));

//  kdlgTopSplitter->activate(kdlg_edit_widget,kdlg_prop_widget);// activate the topSplitter
...
void CKDevelop::initKDlgMenuBar(){
  ...
  kdlg_edit_menu->insertItem(BarIcon("undo"), i18n("U&ndo"), kdlgedit, SLOT(slotEditUndo()), 0, ID_KDLG_EDIT_UNDO);
  kdlg_edit_menu->insertItem(BarIcon("redo"), i18n("R&edo"), kdlgedit, SLOT(slotEditRedo()), 0, ID_KDLG_EDIT_REDO);
  kdlg_edit_menu->insertSeparator();
  kdlg_edit_menu->insertItem(BarIcon("cut"), i18n("C&ut"), kdlgedit, SLOT(slotEditCut()), 0, ID_KDLG_EDIT_CUT);
  kdlg_edit_menu->insertItem(BarIcon("copy"), i18n("&Copy"), kdlgedit, SLOT(slotEditCopy()), 0, ID_KDLG_EDIT_COPY);
  kdlg_edit_menu->insertItem(BarIcon("paste"), i18n("&Paste"), kdlgedit, SLOT(slotEditPaste()), 0, ID_KDLG_EDIT_PASTE);
  kdlg_edit_menu->insertSeparator();
...
// Build-menu entries
  ...
  kdlg_build_menu->insertItem(BarIcon("generate"),i18n("Ge&nerate Complete Sources..."),kdlgedit,
...
//  kdlg_build_menu->insertItem(BarIcon("debugger"),i18n("&Debug..."),this,SLOT(slotBuildDebug()),0,ID_DEBUG_START);
  ...
  kdlg_debug_menu->insertItem(BarIcon("debugger"),    i18n("&Start"),           ID_DEBUG_START);  //this, SLOT(slotBuildDebug()),0,ID_DEBUG_NORMAL);
  kdlg_debug_menu->insertItem(BarIcon("debugger"),    i18n("Start (other)..."), debugPopup, ID_DEBUG_START_OTHER);
  kdlg_debug_menu->insertSeparator();

  kdlg_debug_menu->insertItem(BarIcon("dbgrun"),      i18n("Run"),              ID_DEBUG_RUN);
  kdlg_debug_menu->insertItem(BarIcon("dbgrunto"),    i18n("Run to cursor"),    ID_DEBUG_RUN_CURSOR);
  kdlg_debug_menu->insertItem(BarIcon("dbgnext"),     i18n("Step over"),        ID_DEBUG_NEXT);
  kdlg_debug_menu->insertItem(BarIcon("dbgnextinst"), i18n("Step over instr."), ID_DEBUG_NEXT_INST);
  kdlg_debug_menu->insertItem(BarIcon("dbgstep"),     i18n("Step into"),        ID_DEBUG_STEP);
  kdlg_debug_menu->insertItem(BarIcon("dbgstepinst"), i18n("Step into instr."), ID_DEBUG_STEP_INST);
  kdlg_debug_menu->insertItem(BarIcon("dbgstepout"),  i18n("Step out"),         ID_DEBUG_FINISH);
  kdlg_debug_menu->insertSeparator();

  kdlg_debug_menu->insertItem(BarIcon("dbgmemview"),  i18n("Viewers"),          this, SLOT(slotDebugMemoryView()), 0, ID_DEBUG_MEMVIEW);
  kdlg_debug_menu->insertSeparator();

  kdlg_debug_menu->insertItem(BarIcon("dbgbreak"),    i18n("Interrupt"),        ID_DEBUG_BREAK_INTO);
  kdlg_debug_menu->insertItem(BarIcon("stop_proc"),   i18n("Stop"),             ID_DEBUG_STOP);
  ...
  kdlg_help_menu->insertSeparator();
  kdlg_help_menu->insertItem(BarIcon("kdehelp"), manual,this,SLOT(slotHelpContents()),0 ,ID_HELP_CONTENTS);
  kdlg_help_menu->insertItem(BarIcon("kdehelp"), programming,this,SLOT(slotHelpProgramming()),0 ,ID_HELP_PROGRAMMING);
  kdlg_help_menu->insertItem(BarIcon("kdehelp"), tutorial,this,SLOT(slotHelpTutorial()),0 ,ID_HELP_TUTORIAL);
  kdlg_help_menu->insertItem(BarIcon("kdehelp"), kdelibref,this,SLOT(slotHelpKDELibRef()),0 ,ID_HELP_KDELIBREF);
  kdlg_help_menu->insertItem(BarIcon("kdehelp"), addendum,this,SLOT(slotHelpKDE2DevGuide()),0 ,ID_HELP_KDE2_DEVGUIDE);
  kdlg_help_menu->insertItem(BarIcon("kdehelp"),i18n("C/C++-Reference"),this,SLOT(slotHelpReference()),0,ID_HELP_REFERENCE);
  kdlg_help_menu->insertSeparator();	
...
void CKDevelop::initKDlgToolBar(){
  ...
  toolBar(ID_KDLG_TOOLBAR)->insertSeparator();
	toolBar(ID_KDLG_TOOLBAR)->insertButton(BarIcon("debugger"),ID_DEBUG_START, false, i18n("Debug"));
  ...
  toolBar(ID_KDLG_TOOLBAR)->insertButton(BarIcon("dbgrun"),ID_DEBUG_RUN, false, i18n("Continue with app execution. May start the app"));
  toolBar(ID_KDLG_TOOLBAR)->insertButton(BarIcon("dbgnext"),ID_DEBUG_NEXT, false,i18n("Execute one line of code, but run through functions"));
  toolBar(ID_KDLG_TOOLBAR)->insertButton(BarIcon("dbgstep"),ID_DEBUG_STEP, false,i18n("Execute one line of code, stepping into fn if appropriate"));
  toolBar(ID_KDLG_TOOLBAR)->insertButton(BarIcon("dbgstepout"),ID_DEBUG_FINISH, false,i18n("Execute to end of current stack frame"));
