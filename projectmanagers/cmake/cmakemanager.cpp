...
bool CMakeManager::isValid(const Path& path, bool isFolder, const Filters& filters) const
{
...
QSet<QString> CMakeManager::filterFiles(const QFileInfoList& orig, const Path& folder, const Filters& filters) const
{
    QSet<QString> ret;
    ...
    foreach(const QFileInfo& info, orig)
    {
        ...
        if (isValid(Path(folder, fileName), info.isDir(), filters)) {
            ret.insert(fileName);
        }
    }
    return ret;
...
//             kDebug() << "poor guess";
        ...
        if(folder == project->projectItem())
            folder->includeDirectories(folder);
    ...
    for(QHash<KUrl, KUrl>::iterator it=m_renamed.begin(); it != m_renamed.end(); ) {
        ...
        if(!items.isEmpty()) {
            foreach(ProjectBaseItem* item, items) {
                ///FIXME: port to Path
                if(item->file())
                    emit fileRenamed(Path(it.value()), item->file());
                else
                    emit folderRenamed(Path(it.value()), item->folder());
            }
...
void CMakeManager::reloadFiles(ProjectFolderItem* item, const Filters& filters)
{
    ...
    QSet<QString> entries = filterFiles(entriesL, folderPath, filters);
...
bool CMakeManager::renameFileOrFolder(ProjectBaseItem *item, const Path &newPath)
{
    ...
    if(ret) {
        m_renamed[newPath.toUrl()] = oldUrl;
    } else
