    ...
    : KDevLanguageSupport(&data, parent, name ? name : "PHPSupportPart")
{
  ...
  action = new KAction( i18n("&Run"), "exec",Qt::Key_F9,
			this, SLOT(slotRun()),
  ...
  Q3WhatsThis::add(m_phpErrorView, i18n("<b>PHP problems</b><p>This view shows PHP parser warnings, errors, and fatal errors."));
  ...
  new KAction(i18n("Complete Text"), Qt::CTRL+Qt::Key_Space, m_codeCompletion, SLOT(cursorPositionChanged()),
...
void PHPSupportPart::projectConfigWidget(KDialogBase *dlg){
  Q3VBox *vbox = dlg->addVBoxPage(i18n( "PHP Specific" ), i18n("PHP Settings"), BarIcon( "source", KIcon::SizeMedium ));
...
bool PHPSupportPart::validateConfig(){
  if(!configData->validateConfig()){
    ...
    Q3VBox *vbox = dlg.addVBoxPage(i18n("PHP Settings"));
    PHPConfigWidget* w = new PHPConfigWidget(configData,vbox, "php config widget");
    connect( &dlg, SIGNAL(okClicked()), w, SLOT(accept()) );
    dlg.exec();
  }
...
//   kdDebug(9018) << "phpSupportPart::customEvent(" << ev->type() << ")" << endl;
   ...
   if ( ev->type() == int(Event_FileParsed) ){
      ...
      Q3ValueList<Action *> actions = event->actions();
      Q3ValueList<Action *>::ConstIterator it = actions.begin();
