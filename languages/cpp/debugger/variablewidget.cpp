      ...
      recentExpressions_(0)
{
    ...
    connect( this, SIGNAL(toggleRadix(Q3ListViewItem*)), SLOT(slotToggleRadix(Q3ListViewItem*)) );
...
void VariableTree::slotContextMenu(KListView *, Q3ListViewItem *item)
{
    ...
    {
        KMenu popup(this);
        popup.addTitle(item->text(VarNameCol));
        QAction* idRemember = 0L;
        QAction* idRemove = 0L;
        QAction* idReevaluate = 0L;
        QAction* idWatch = 0L;
        ...
        {
            idRemember = popup.addAction(
                SmallIcon("pencil"), i18n("Remember Value"));
        }

        if (dynamic_cast<WatchRoot*>(root)) {
            idRemove = popup.addAction( 
                SmallIcon("editdelete"), i18n("Remove Watch Variable") );
        } else if (root != recentExpressions_) {
            idWatch = popup.addAction(
                i18n("Watch Variable"));
        }
        if (root == recentExpressions_) {
            idReevaluate = popup.addAction( 
                SmallIcon("reload"), i18n("Reevaluate Expression") );
            idRemove = popup.addAction( 
                SmallIcon("editdelete"), i18n("Remove Expression") );
        }

        QAction* idToggleWatch = popup.addAction( i18n("Toggle Watchpoint") );
        QAction* idToggleRadix = popup.addAction( i18n("Toggle Hex/Decimal") );
        QAction* idCopyToClipboard = popup.addAction(
    ...
    else if (item == recentExpressions_)
    {
        ...
        QAction* idRemove = popup.addAction(
            SmallIcon("editdelete"), i18n("Remove All"));
        QAction* idReevaluate = popup.addAction(
        ...
        {
            for(Q3ListViewItemIterator it(item); *it; ++it)
            {
...
void VariableTree::maybeTip(const QPoint &p)
{
    ...
    if ( item )
    {
        ...
        if ( r.isValid() )
            QToolTip::showText( r, item->tipText() );
    }*/
}
 ...
 */
void VariableTree::slotToggleRadix(Q3ListViewItem * item)
{
...
void VarItem::updateValue(char *buf)
{
    ...
    GDBParser::getGDBParser()->parseValue(this, r.toLatin1());
    setActive();
}
...
void VarItem::handleSpecialTypes()
{
    ...
    if (qstring.exactMatch(type)) {
        ...
        emit ((VariableTree*)listView())->expandUserItem(
            this,
            Q3CString().sprintf("(($len=($data=%s.d).len)?*((char*)&$data.unicode[0])@($len>100?200:$len*2):\"\")",
