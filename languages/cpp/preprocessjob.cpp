...
void PreprocessJob::run(ThreadWeaver::JobPointer self, ThreadWeaver::Thread* thread)
{
    ...
    //qCDebug(CPP) << "Started pp job" << this << "parse" << parentJob()->parseJob() << "parent" << parentJob();

    qCDebug(CPP) << "PreprocessJob: preprocessing" << parentJob()->document().str();
    if( parentJob()->parentPreprocessor() )
      qCDebug(CPP) << "PARENT:" << parentJob()->parentPreprocessor()->parentJob()->document().toUrl();
            ...
            bool needsUpdate = cppEnv->needsUpdate();
              if(!cppEnv->missingIncludeFiles().isEmpty() && !needsUpdate) {
                for(Cpp::ReferenceCountedStringSet::Iterator it = cppEnv->missingIncludeFiles().iterator(); it; ++it)
                  qCDebug(CPP) << updatingEnvironmentFile->url().str() << "has missing include:" << (*it).str();
    ...
    for (auto i: parentJob()->masterJob()->indexedIncludePaths()) {
        const QUrl includeUrl = i.toUrl();
        QString include = includeUrl.toLocalFile();
        QFileInfo info(include);
        if (info.isFile() && CppUtils::isHeader(includeUrl)) {
    ...
    if(Cpp::EnvironmentManager::self()->matchingLevel() <= Cpp::EnvironmentManager::Naive && !m_headerSectionEnded && !m_firstEnvironmentFile->headerGuard().isEmpty()) {
      if(macroNamesAtBeginning.contains(m_firstEnvironmentFile->headerGuard())) {
        ...
        qCDebug(CPP) << "Re-processing header with the header-guard disabled";
    ...
    if(!m_headerSectionEnded) {
      ifDebug( qCDebug(CPP) << parentJob()->document().str() << ": header-section was not ended"; )
    ...
    if( m_secondEnvironmentFile ) {
        //qCDebug(CPP) << parentJob()->document().str() << "Merging content-environment file into header environment-file";
        KDevelop::DUChainReadLocker readLock(KDevelop::DUChain::lock());
...
/*        qCDebug(CPP) << "Macros:";
        for( rpp::Environment::EnvironmentMap::const_iterator it = m_currentEnvironment->environment().begin(); it != m_currentEnvironment->environment().end(); ++it ) {
            qCDebug(CPP) << (*it)->name.str() << "                  from: " << (*it)->file << ":" << (*it)->sourceLine;
        }*/
    }
    ifDebug( qCDebug(CPP) << "PreprocessJob: finished" << parentJob()->document().str(); )
...
void PreprocessJob::headerSectionEndedInternal(rpp::Stream* stream)
{
    ...
    ifDebug( qCDebug(CPP) << parentJob()->document().str() << "PreprocessJob::headerSectionEnded, " << parentJob()->includedFiles().count() << " included in header-section" << "upcoming identity-offset:" << m_pp->branchingHash()*19; )
        ...
        if(content) {
            ...
            if(contentEnvironment->matchEnvironment(m_currentEnvironment) && !CppUtils::needsUpdate(contentEnvironment, localPath, parentJob()->includePathUrls()) && (!parentJob()->masterJob()->needUpdateEverything() || parentJob()->masterJob()->wasUpdated(content)) && (content->parsingEnvironmentFile()->featuresSatisfied(parentJob()->minimumFeatures()) && content->parsingEnvironmentFile()->featuresSatisfied(parentJob()->slaveMinimumFeatures()))
              ...
              ///           However when full environment management is enabled this is needed, as the same content may be shared for multiple proxy contexts.
                ...
                ifDebug( qCDebug(CPP) << "closing data-stream, body does not need to be processed"; )
                closeStream = true;
                ...
                ifDebug( qCDebug(CPP) << "updating content-context"; )
            ...
            ifDebug( qCDebug(CPP) << "could not find a matching content-context"; )
        }
...
rpp::Stream* PreprocessJob::sourceNeeded(QString& fileName, IncludeType type, int sourceLine, bool skipCurrentPath)
{
    ...
    if(0){
      ...
      if(currentDepth > maxIncludeDepth) {
        qCDebug(CPP) << "maximum depth reached while including" << fileName << "into" << parentJob()->document().str();
        return 0;
      }
    }
    ...
    ifDebug( qCDebug(CPP) << "PreprocessJob" << parentJob()->document().str() << ": searching for include" << fileName; )
    ...
    if (info.isFile() && info.isAbsolute()) {
        ...
        included.first = Path(fileName);
        included.second = included.first.parent();
    } else {
    ...
    if (includedFile.isValid()) {
        ...
        ifDebug( qCDebug(CPP) << "PreprocessJob" << parentJob()->document().str()
                              << "(" << m_currentEnvironment->environment().size() << "macros): found include-file"
...
//               }
            ...
            if(includedContext) {
              ...
              if( includedEnvironment ) {
                ...
                if(!includedEnvironment->headerGuard().isEmpty() && m_currentEnvironment->macroNameSet().contains(includedEnvironment->headerGuard())) {
                  ...
                  qCDebug(CPP) << "forbidding update of" << includedFile;
                  updateNeeded = false;
                }
              }
            }
        ...
        if( includedContext && (updateForbidden || (!updateNeeded && (!parentJob()->masterJob()->needUpdateEverything() || parentJob()->masterJob()->wasUpdated(includedContext)))) ) {
            ifDebug( qCDebug(CPP) << "PreprocessJob" << parentJob()->document().str() << ": took included file from the du-chain" << fileName; )
            ...
            if( environmentFile ) {
                ...
                ifDebug( qCDebug(CPP) << "PreprocessJob" << parentJob()->document().str() << "Merging included file into environment-file"; )
            } else {
                ifDebug( qCDebug(CPP) << "preprocessjob: included file" << includedFile << "found in du-chain, but it has no parse-environment information, or it was not parsed by c++ support"; )
            }
        } else {
            if(updateNeeded)
              qCDebug(CPP) << "PreprocessJob" << parentJob()->document().str() << ": need to update" << includedFile;
            else if(parentJob()->masterJob()->needUpdateEverything() && includedContext)
              qCDebug(CPP) << "PreprocessJob" << parentJob()->document().str() << ": needUpateEverything, updating" << includedFile;
            else
              qCDebug(CPP) << "PreprocessJob" << parentJob()->document().str() << ": no fitting entry for" << includedFile << "in du-chain, parsing";
...
/*            if( updateNeeded && !parentJob()->masterJob()->needUpdateEverything() ) {
              ...
              qCDebug(CPP) << "Marking every following encountered context to be updated";
            ...
            if(slaveJob->duChain())
              ...
              qCDebug(CPP) << "parse-job for" << includedFile << "did not return a top-context";
            delete slaveJob;
        }
        ifDebug( qCDebug(CPP) << "PreprocessJob" << parentJob()->document().str() << "(" << m_currentEnvironment->environment().size() << "macros)" << ": file included"; )
        ...
        p->setFinalLocation(DocumentRange(parentJob()->document(), KTextEditor::Range(sourceLine,0, sourceLine+1,0)));
        p->setSolutionAssistant(IAssistant::Ptr(new Cpp::MissingIncludePathAssistant(parentJob()->masterJob()->document(), fileName)));
...
bool PreprocessJob::checkAbort()
{
  if(ICore::self()->shuttingDown()) {
    qCDebug(CPP) << "The application is shutting down";
    return true;
  }
  ...
  if (!ICore::self()->languageController()->language("C++")) {
    ...
    if (CPPParseJob* parent = parentJob()) {
        ...
        qCWarning(CPP) << "Parent job disappeared!!" ;
        m_success = false;
