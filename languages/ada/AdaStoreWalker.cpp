/* $ANTLR 2.7.7 (20061129): "expandedada.store.g" -> "AdaStoreWalker.cpp"$ */
	...
	try {      // for error handling
		...
		for (;;) {
			...
			if ((_tokenSet_2.member(_t->getType()))) {
				...
				if ( _cnt464>=1 ) { goto _loop464; } else {throw ANTLR_USE_NAMESPACE(antlr)NoViableAltException(ANTLR_USE_NAMESPACE(antlr)RefAST(_t));}
			}
	...
	try {      // for error handling
		...
		for (;;) {
			...
			if ((_t->getType() == IDENTIFIER || _t->getType() == CHARACTER_LITERAL)) {
				...
				if ( _cnt213>=1 ) { goto _loop213; } else {throw ANTLR_USE_NAMESPACE(antlr)NoViableAltException(ANTLR_USE_NAMESPACE(antlr)RefAST(_t));}
			}
	...
	try {      // for error handling
		...
		for (;;) {
			...
			if ((_t->getType() == DISCRIMINANT_ASSOCIATION)) {
				...
				if ( _cnt243>=1 ) { goto _loop243; } else {throw ANTLR_USE_NAMESPACE(antlr)NoViableAltException(ANTLR_USE_NAMESPACE(antlr)RefAST(_t));}
			}
...
void AdaStoreWalker::statements(RefAdaAST _t) {
	...
	try {      // for error handling
		...
		for (;;) {
			...
			default:
			{
				if ( _cnt338>=1 ) { goto _loop338; } else {throw ANTLR_USE_NAMESPACE(antlr)NoViableAltException(ANTLR_USE_NAMESPACE(antlr)RefAST(_t));}
			}
			}
			_cnt338++;
		}
	...
	try {      // for error handling
		...
		for (;;) {
			...
			if ((_tokenSet_4.member(_t->getType()))) {
				...
				if ( _cnt440>=1 ) { goto _loop440; } else {throw ANTLR_USE_NAMESPACE(antlr)NoViableAltException(ANTLR_USE_NAMESPACE(antlr)RefAST(_t));}
			}
	...
	try {      // for error handling
		...
		for (;;) {
			...
			if ((_t->getType() == CASE_STATEMENT_ALTERNATIVE)) {
				...
				if ( _cnt362>=1 ) { goto _loop362; } else {throw ANTLR_USE_NAMESPACE(antlr)NoViableAltException(ANTLR_USE_NAMESPACE(antlr)RefAST(_t));}
			}
