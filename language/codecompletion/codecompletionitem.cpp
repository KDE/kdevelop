...
void NormalDeclarationCompletionItem::execute(KTextEditor::Document* document, const KTextEditor::Range& word) {
  ...
  if( declaration && dynamic_cast<AbstractFunctionDeclaration*>(declaration.data()) ) { //Do some intelligent stuff for functions with the parens:
    ...
    if( declaration && declaration->type<FunctionType>() && declaration->type<FunctionType>()->arguments().count() )
      haveArguments = true;
      ...
      if( currentMatchContext && currentMatchContext->asItem()) {
        ...
        if( contextItem.asItem() && contextItem.declaration && contextItem.completionContext && contextItem.completionContext->memberAccessOperation() == ::CodeCompletionContext::FunctionCallAccess && contextItem.listOffset < contextItem.completionContext->functions().count() )
        {
          ...
          if( f.function.isValid() && f.function.isViable() && f.function.declaration() && f.function.declaration()->type<FunctionType>() && f.function.declaration()->type<FunctionType>()->arguments().count() > f.matchedArguments ) {
            ...
            int quality = ( conv.implicitConversion( effectiveType(dec), f.function.declaration()->type<FunctionType>()->arguments()[f.matchedArguments], true )  * 10 ) / ::MaximumConversionResult;
...
/*          QualifiedIdentifier id = dec->qualifiedIdentifier();
    ...
    case CodeCompletionModel::CompletionRole: {
      ...
      if (dec->abstractType()) {
        if (CVType* cv = dynamic_cast<CVType*>(dec->abstractType().data())) {
          if (cv->isConstant())
