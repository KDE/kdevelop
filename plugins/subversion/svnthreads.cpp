...
SubversionThread::SubversionThread( SvnJobBase::Type actionType, SvnJobBase *parent )
    : QThread(parent), d( new Private ) /*, m_triedTermination(false)*/
{
    ...
    if ( err ) {
        kDebug(9500) << "SvnJobBaseBase::SvnJobBaseBase() create_context ERROR";
        setErrorMsg( QString::fromUtf8(err->message) );
        return;
    }
    ...
    if ( !err ) {
        ...
        kDebug(9500) << "SvnJobBaseBase:: svn_config_ensure failed:";
    }
...
SvnJobBase::Type SubversionThread::type()
{
    return d->m_type;
}

SvnJobBase* SubversionThread::kjob()
{
    return d->m_kjob;
}
                                    ...
                                    apr_pool_t *pool)
{
    ...
    SubversionThread *p = static_cast<SubversionThread*>(baton);
    ...
    *(p->loginInfo()) = loginInfo;
    ...
    ret->username = apr_pstrdup(pool, userName.toUtf8().data());
    ret->password = apr_pstrdup(pool, passWord.toUtf8().data());
    if(may_save)
                                    ...
                                    apr_pool_t *pool)
{
    ...
    SubversionThread *th = static_cast<SubversionThread*>(baton);
...
{
    SubversionThread *thread = static_cast<SubversionThread*>(baton);
                                    ...
                                    apr_pool_t *pool )
{
    *tmp_file = NULL;
    ...
    if( !thread->commitMessage().isEmpty() ){
        char *msg = apr_pstrdup( pool, thread->commitMessage().toUtf8().data() );
        *log_msg = msg;
        return SVN_NO_ERROR;
    }
    ...
    if( isAccepted ){
        svn_string_t *string = svn_string_create( fetchedMsg.toUtf8().data(), pool );
        *log_msg = string->data;
    } else {
        *log_msg = NULL;
...
{
//     kjob()->emitResult(); // called at SvnJobBase's slot. don't call here again.
}
...
void SubversionThread::customEvent( QEvent *event )
{
    switch( event->type() ){
        case SvnInterThreadPromptEvent::LOGIN_SERVERTRUSTPROMPT : {
            ...
            SvnServerCertInfo *recvCert= static_cast<SvnServerCertInfo *>(ev->m_data);
...
SvnBlameJob::SvnBlameJob(  const KUrl& path_or_url,
                        ...
                        SvnJobBase::Type actionType, SvnJobBase *parent )
...
void SvnBlameJob::run()
{
    ...
    QByteArray path = m_pathOrUrl.pathOrUrl().toUtf8();
    const char* path_or_url= apr_pstrdup( subpool, path.data() );
    ...
    svn_error_t *err = svn_client_blame( path_or_url, &rev1, &rev2, receiver, this, ctx(), subpool );
    if ( err ){
        setErrorMsgExt( err );
...
{
    SvnBlameJob *job = static_cast<SvnBlameJob*>(baton);
    SvnBlameHolder holder;
    holder.lineNo = line_no;
    holder.revNo = rev;
    holder.author = author;
    holder.date = QDateTime::fromString( QString::fromUtf8( date ), Qt::ISODate );
...
//     peg_rev = pegrev.revision();
    ...
    for ( QList<KUrl>::iterator it = urls.begin(); it != urls.end() ; ++it ) {
        KUrl nurl = *it;
        QByteArray urlpath = nurl.pathOrUrl().toUtf8();
//         nurl.setProtocol( "file" );
        const char *path =
            apr_pstrdup( subpool, svn_path_canonicalize( urlpath.data(), subpool ) );
        kDebug(9500) << path;
...
{
    SvnLogviewJob *p = static_cast<SvnLogviewJob*>(baton);
    SvnLogHolder holder;
    holder.rev = revision;
    holder.author = author;
    holder.date = QDateTime::fromString( QString::fromUtf8( date ), Qt::ISODate );;
    ...
    if( changed_paths != NULL ){
        ...
        for (hi = apr_hash_first(pool, changed_paths); hi; hi = apr_hash_next(hi)) {
            ...
            SvnChangedPath scp;
            scp.action = cp->action;
            scp.path = QString::fromUtf8( static_cast<const char*>( pathkey ) );
            scp.copyFromPath = QString::fromUtf8( cp->copyfrom_path );
            scp.copyFromRevision = cp->copyfrom_rev;
        }
...
//     svn_client_commit_info_t *commit_info = NULL;
    ...
    for ( QList<KUrl>::iterator it = m_urls.begin(); it != m_urls.end() ; ++it ) {
        ...
        (*(( const char ** )apr_array_push(( apr_array_header_t* )targets)) ) =
                svn_path_canonicalize( urlpath.data(), subpool );
    }
...
SvnStatusJob::SvnStatusJob( const KUrl &wcPath, const SvnRevision &rev,
                            ...
                            SvnJobBase::Type type, SvnJobBase *parent )
...
void SvnStatusJob::run()
{
    ...
    QByteArray urlpath = m_wcPath.path().toUtf8();
...
void SvnStatusJob::statusReceiver( void *baton, const char *path, svn_wc_status2_t *status )
{
    ...
    SvnStatusJob *job = static_cast<SvnStatusJob*>(baton);
...
void SvnAddJob::run()
{
    ...
    for( QList<KUrl>::iterator it = m_wcPaths.begin(); it != m_wcPaths.end(); ++it ){
        ...
        svn_error_t *err = svn_client_add2( svn_path_canonicalize( urlpath.data(), subpool ),
...
SvnDeleteJob::SvnDeleteJob( const KUrl::List &urls, bool force, SvnJobBase::Type type, SvnJobBase *parent )
    : SubversionThread( type, parent )
...
SvnInfoJob::SvnInfoJob( const KUrl &pathOrUrl,
                        ...
                        bool recurse, SvnJobBase::Type type, SvnJobBase *parent  )
    : SubversionThread( type, parent )
...
{
    SvnInfoJob *job = static_cast<SvnInfoJob*>(baton) ;
    if( !job )
        return SVN_NO_ERROR;
    ...
    holder.repoUrl = KUrl( info->repos_root_URL );
    holder.repouuid = QString::fromUtf8( info->repos_UUID );
    holder.lastChangedRev = info->last_changed_rev;
    holder.lastChangedAuthor = QString::fromUtf8( info->last_changed_author );
...
void SvnInfoJob::run()
{
    ...
    QByteArray urlpath = m_pathOrUrl.pathOrUrl().toUtf8();
    svn_error_t *err = svn_client_info( urlpath.data(),
                                        &peg_rev, &revision,
...
void SvnDiffJob::run()
{
    ...
    if ( m_pathOrUrl1.protocol() == "file" ) {
        QByteArray urlpath = m_pathOrUrl1.path().toUtf8();
        path1 = svn_path_canonicalize( apr_pstrdup( subpool, urlpath.data() ), subpool );
    } else {
        QByteArray urlpath = m_pathOrUrl1.pathOrUrl().toUtf8();
        path1 = apr_pstrdup( subpool, urlpath.data() );
    }
    if ( m_pathOrUrl2.protocol() == "file" ) {
        QByteArray urlpath = m_pathOrUrl2.path().toUtf8();
        path2 = svn_path_canonicalize( apr_pstrdup( subpool, urlpath.data() ), subpool );
    } else {
        QByteArray urlpath = m_pathOrUrl2.pathOrUrl().toUtf8();
        path2 = apr_pstrdup( subpool, urlpath.data() );
    }
...
//     QString tmpPath = m_tmpDir->name();
    ...
    QByteArray outpath = QString( tmpPath + "svndiffout_XXXXXX" ).toUtf8();
    QByteArray errpath = QString( tmpPath + "svndifferr_XXXXXX" ).toUtf8();
    out_name = apr_pstrdup( pool(), outpath.data() );
    err_name = apr_pstrdup( pool(), errpath.data() );
...
void SvnImportJob::run()
{
    ...
    QByteArray bpath = m_path.path().toUtf8();
    QByteArray burl = m_url.pathOrUrl().toUtf8();
    const char *path = apr_pstrdup( subpool,
                svn_path_canonicalize( bpath.data(), subpool ) );
    const char *url = apr_pstrdup( subpool,
                svn_path_canonicalize( burl.data(), subpool ) );
...
void SvnCheckoutJob::run()
{
    ...
    QByteArray url = m_servUrl.url().toUtf8();
    QByteArray wc = m_wcRoot.path().toUtf8();
    const char *reposUrl = apr_pstrdup( subpool, url.data() );
    const char *wcRoot = apr_pstrdup( subpool,
                        svn_path_canonicalize( wc.data(), subpool ) );
...
void SvnRevertJob::run()
{
    ...
    for ( QList<KUrl>::iterator it = d->m_paths.begin() ; it != d->m_paths.end() ; ++it ) {
        KUrl nurl = *it;
        nurl.setProtocol( "file" );
        QByteArray url = nurl.path().toUtf8();
        *(( const char ** )apr_array_push(( apr_array_header_t* )targets)) =
                svn_path_canonicalize( url.data(), subpool );
        kDebug(9500) << "canonicalized path:" << nurl.path();
    }
...
SvnCopyJob::SvnCopyJob( const KUrl& srcPathOrUrl, const SvnRevision &srcRev,
                        const KUrl& dstPathOrUrl, SvnJobBase::Type type, SvnJobBase *parent )
    : SubversionThread( type, parent ), d( new Private )
{
...
void SvnCopyJob::run()
{
    ...
    QByteArray srcurl = d->m_srcPathOrUrl.pathOrUrl().toUtf8();
    QByteArray dsturl = d->m_dstPathOrUrl.pathOrUrl().toUtf8();
...
SvnMoveJob::SvnMoveJob( const KUrl& srcPathOrUrl, const KUrl& dstPathOrUrl,
                bool force, SvnJobBase::Type type, SvnJobBase *parent )
    : SubversionThread( type, parent ), d( new Private )
{
    d->m_srcPathOrUrl = srcPathOrUrl;
    d->m_dstPathOrUrl = dstPathOrUrl;
...
void SvnCatJob::run()
{
    ...
    QByteArray url = m_path_or_url.pathOrUrl().toUtf8();
    const char* path_or_url = apr_pstrdup( pool(),
            svn_path_canonicalize( url.data(), pool() ) );
...
{
    SvnCatJob *job = static_cast<SvnCatJob*>(baton);
