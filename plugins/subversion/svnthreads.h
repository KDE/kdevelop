...
class SubversionThread : public QThread
{
    Q_OBJECT
public:
    SubversionThread( SvnJobBase::Type actionType, SvnJobBase *parent );
    virtual ~SubversionThread();

    SvnJobBase::Type type();
    SvnJobBase* kjob();
...
protected:
    ...
    virtual void run() = 0;
    /// set error message into parent SvnJobBase
...
{
public:
    SvnDeleteJob( const KUrl::List &urls, bool force, SvnJobBase::Type type, SvnJobBase *parent );
protected:
    virtual void run();
