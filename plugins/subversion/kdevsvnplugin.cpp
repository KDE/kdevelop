...
void KDevSvnPlugin::ctxHistory()
{
    ...
    KDevelop::VcsJob *job = log(ctxUrlList.first(), start, 0);
    KDialog* dlg = new KDialog();
    dlg->setButtons(KDialog::Close);
    dlg->setCaption(i18n("Subversion Log (%1)", ctxUrlList.first().toLocalFile()));
    KDevelop::VcsEventWidget* logWidget = new KDevelop::VcsEventWidget(ctxUrlList.first(), job, dlg);
...
void KDevSvnPlugin::ctxCommit()
{
    ...
    dlg->setCommitCandidates(ctxUrlList);
...
void KDevSvnPlugin::ctxUpdate()
{
    ...
    KDevelop::ICore::self()->runController()->registerJob(update(ctxUrlList, rev));
}

void KDevSvnPlugin::ctxAdd()
{
    KUrl::List const & ctxUrlList = m_common->contextUrlList();
    KDevelop::ICore::self()->runController()->registerJob(add(ctxUrlList));
}

void KDevSvnPlugin::ctxRemove()
{
    KUrl::List const & ctxUrlList = m_common->contextUrlList();
    KDevelop::ICore::self()->runController()->registerJob(remove(ctxUrlList));
}
...
void KDevSvnPlugin::ctxRevert()
{
    ...
    KDevelop::ICore::self()->runController()->registerJob(revert(ctxUrlList));
}
...
void KDevSvnPlugin::ctxDiffHead()
{
    ...
    KDevelop::VcsJob* job = diff(ctxUrlList.first(), srcRev, dstRev);
