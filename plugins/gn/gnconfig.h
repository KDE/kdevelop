...
QString absoluteFilePath(const QString &rootPath, const QString &path);
QString relativeFilePath(const QString &path);

struct BuildDir {
    KDevelop::Path buildDir;
    KDevelop::Path gnExecutable;
    QString gnArgs;
...
GNConfig getGNConfig(KDevelop::IProject* project);
void writeGNConfig(KDevelop::IProject* project, const GNConfig& cfg);
