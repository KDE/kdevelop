...
void PatchReviewPlugin::seekHunk( bool forwards, const QUrl& fileName ) {
    try {
        ...
        for ( int a = 0; a < m_modelList->modelCount(); ++a ) {
            const KompareDiff2::DiffModel* model = m_modelList->modelAt( a );
...
void PatchReviewPlugin::addHighlighting( const QUrl& highlightFile, IDocument* document ) {
    try {
        ...
        for ( int a = 0; a < modelList()->modelCount(); ++a ) {
            KompareDiff2::DiffModel* model = modelList()->modelAt( a );
            if ( !model )
                continue;
...
void PatchReviewPlugin::highlightPatch() {
    try {
        ...
        for ( int a = 0; a < modelList()->modelCount(); ++a ) {
            const KompareDiff2::DiffModel* model = modelList()->modelAt( a );
            if ( !model )
                continue;
...
void PatchReviewPlugin::updateKompareModel() {
    ...
    try {
        ...
        for( int i = 0; i < m_modelList->modelCount(); i++ ) {
            const KompareDiff2::DiffModel* model = m_modelList->modelAt( i );
...
QUrl PatchReviewPlugin::urlForFileModel( const KompareDiff2::DiffModel* model )
{
...
{
    qRegisterMetaType<const KompareDiff2::DiffModel*>( "const KompareDiff2::DiffModel*" );
