...
void HgInitTest::addFiles()
{
    ...
    HgJob* j = m_proxy->add(QString(HGTEST_BASEDIR), KUrl::List(QStringList(HG_TESTFILE_NAME)));
    QVERIFY( j );
...
void HgInitTest::commitFiles()
{
    ...
    HgJob* j = m_proxy->commit(QString(HGTEST_BASEDIR), QString("KDevelop's Test commit"),
    ...
    //since we commited the file to the "pure" repository, .hg/refs/heads/master should exist
    //TODO: maybe other method should be used
    ...
    j = m_proxy->commit(QString(HGTEST_BASEDIR), QString("KDevelop's Test commit2"));
    QVERIFY( j );
