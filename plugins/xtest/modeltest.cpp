...
void ModelTest::runAllTests()
{
    ...
    kDebug() << "nonDestructiveBasicTest: OK";
    rowCount();
    kDebug() << "rowCount:                OK";
    columnCount();
    kDebug() << "columnCount:             OK";
    hasIndex();
    kDebug() << "hasIndex:                OK";
    index();
    kDebug() << "index:                   OK";
...
void ModelTest::parent()
{
    ...
    if (model->rowCount(topIndex) > 0) {
        QModelIndex childIndex = model->index(0, 0, topIndex);
        kDebug() << "child  " << model->data(childIndex);
        kDebug() << "parent " << model->data(topIndex);
        kDebug() << "childchild " << model->data(model->index(0,0,childIndex));
        kDebug() << "root2  " << model->data(model->index(1,0,QModelIndex()));
        kDebug() << "child2 " << model->data(model->index(1,0,topIndex));
...
void ModelTest::rowsInserted(const QModelIndex & parent, int start, int end)
{
    ...
    kDebug() << "oldSize " << c.oldSize << " end " << end 
             << " start " << start << " rowcount " << model->rowCount(parent);
