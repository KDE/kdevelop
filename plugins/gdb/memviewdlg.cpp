...
{
    m_appRunning = !state.testFlag(s_appNotStarted);
    enableOrDisable();
}
...
void MemoryView::contextMenuEvent(QContextMenuEvent *e)
{
    ...
    write->setEnabled(m_appRunning && m_memViewView->isModified());

    QAction* range = menu.addAction(i18nc("@action:inmenu", "Change Memory Range"));
    range->setEnabled(m_appRunning && !m_rangeSelector->isVisible());
...
{
    const auto enable = m_appRunning && !m_rangeSelector->startAddressLineEdit->text().isEmpty();
