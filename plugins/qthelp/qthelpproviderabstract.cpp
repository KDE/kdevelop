...
IDocumentation::Ptr QtHelpProviderAbstract::documentationForDeclaration(Declaration* dec) const
{
    if (dec) {
...

        {
            ...
            if (qualifiedId.count() > 1 && dec->type<EnumeratorType>()) {
                const auto enumeratorId = qualifiedId.at(qualifiedId.count() - 1).toString();
                const auto enumId = qualifiedId.at(qualifiedId.count() - 2).toString();
                fallbackId = id;
