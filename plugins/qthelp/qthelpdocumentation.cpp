...
QString descriptionFromHtmlData(const QString& fragment, const QString& dataString)
{
    ...
    if (!fragment.isEmpty()) {
        const QString exp = QString(QLatin1String("< a name = ['\"]") + fragment + QLatin1String("['\"] > < / a >"))
    ...
    const QString exp =
        QStringLiteral("< a name = ['\"]((\\S)*)['\"] > < / a >").replace(QLatin1Char(' '), optionalSpace);
...

    {
        ...
        const QString link = QStringLiteral("< a href = ['\"].*?['\"]").replace(QLatin1Char(' '), optionalSpace);
