...
bool GitExecutor::isValidDirectory(const KUrl & dirPath)
{
    ...
    if (job)
    {
        ...
        {
            kDebug(9525) << "Dir:" << dirPath << " is inside work tree of git" ;
            return true;
        }
    }
    kDebug(9525) << "Dir:" << dirPath.path() << " is not inside work tree of git" ;
    return false;
}
...
QString GitExecutor::curBranch(const QString &repository)
{
    ...
    if (job)
    {
        kDebug(9525) << "Getting branch list";
        job->exec();
    }
    QString branch;
    if (job->status() == KDevelop::VcsJob::JobSucceeded)
    ...
    kDebug(9525) << "Current branch is: " << branch;
    return branch;
}
...
QList<DVCScommit> GitExecutor::getAllCommits(const QString &repo)
{
    ...
    for(int i = 0; i < commits.count(); ++i)
    {
        ...
        {
            kDebug(9525) << "commit found in " << commits[i];
            item.setCommit(commits[i].section(' ', 1, 1).trimmed());
//             kDebug(9525) << "commit is: " << commits[i].section(' ', 1);
...
//             kDebug(9525) << "author is: " << commits[i].section("Author: ", 1);

            item.setDate(commits[++i].section("Date:   ", 1).trimmed());
//             kDebug(9525) << "date is: " << commits[i].section("Date:   ", 1);
...
//             kDebug(9525) << "log is: " << log;
            ...
            {
                kDebug(9525)<<"commit: " << item.getCommit();
                ...
                if (!branchesShas[i].contains(parent))
                {
                        ...
                        f_iter != commitList.end(); ++f_iter)
                    {
                        if (parent == f_iter->getCommit())
                        {
                            ...
                            kDebug(9525) << parent << " is parent of " << commit;
                            kDebug(9525) << f_iter->getCommit() << " is merge";
                            parent_checked = true;
                            break;
                        }
                        else
