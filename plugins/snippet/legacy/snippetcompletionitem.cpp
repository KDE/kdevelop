...
void SnippetCompletionItem::execute( KTextEditor::View* view, const KTextEditor::Range& word )
{
    ...
    if ( view->selection() ) {
        values["selection"] = view->document()->text(view->selectionRange());
    }
    ...
    if ( templateIface2 )
    {
        if ( word != view->selectionRange() ) {
            view->document()->removeText(word);
        }
        templateIface2->insertTemplateText(word.start(), m_snippet, values, m_repo->registeredScript());
        return;
    }
    KTextEditor::TemplateInterface* templateIface = qobject_cast<KTextEditor::TemplateInterface*>(view);
    if ( templateIface ) {
        templateIface->insertTemplateText(word.start(), m_snippet, values);
        return;
    }

    view->document()->replaceText(word, m_snippet);
}
