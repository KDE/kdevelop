...
SubclassingDlg::SubclassingDlg(JavaSupportPart* javaSupport, const QString &formFile,QStringList &newFileNames,
                               QWidget* parent, const char* name,bool modal, WFlags fl)
: SubclassingDlgBase(parent,name,modal,fl),
m_newFileNames(newFileNames), m_javaSupport( javaSupport )
//=================================================
{
  m_formFile = formFile;
  readUiFile();
...
SubclassingDlg::SubclassingDlg(JavaSupportPart* javaSupport, const QString &formFile,const QString &filename,QStringList &dummy,
                               QWidget* parent, const char* name,bool modal, WFlags fl)
: SubclassingDlgBase(parent,name,modal,fl),
m_newFileNames(dummy), m_javaSupport( javaSupport )
//=================================================
{
  m_formFile = formFile;
  ...
  RefJavaAST translationUnit = m_javaSupport->backgroundParser()->translationUnit( filename + ".h" );
  if( !translationUnit ){
      m_javaSupport->backgroundParser()->unlock();
      m_javaSupport->backgroundParser()->addFile( filename + ".h" );
      ...
      m_javaSupport->backgroundParser()->lock();
      translationUnit = m_javaSupport->backgroundParser()->translationUnit( filename + ".h" );
  }
  if( translationUnit ){
      JavaStoreWalker w;
      w.setFileName( filename + ".h" );
      w.setClassStore( &classcontainer );
      w.compilationUnit( translationUnit );
  }
  m_javaSupport->backgroundParser()->unlock();
...
//===================================================================
{
  ...
  if (canBeModal)
  {
    ...
    replace(buffer,"$CAN_BE_MODAL_JAVA1$",", bool modal");
    replace(buffer,"$CAN_BE_MODAL_JAVA2$",", modal");
  }
  else
  {
    replace(buffer,"$CAN_BE_MODAL_H$","");
    replace(buffer,"$CAN_BE_MODAL_JAVA1$","");
    replace(buffer,"$CAN_BE_MODAL_JAVA2$","");
  }
...
//===========================
{
  ...
  if (m_creatingNewSubclass)
    loadBuffer(buffer,::locate("data", "kdevjavasupport/subclassing/subclass_template.h"));
  else
  ...
  if (reformat_box->isChecked())
    buffer = m_javaSupport->sourceFormatter()->formatSource(buffer);
  ...
  if (m_creatingNewSubclass)
    loadBuffer(buffer,::locate("data", "kdevjavasupport/subclassing/subclass_template.java"));
  else
    loadBuffer(buffer,m_filename+".java");
  replaceKeywords(buffer,m_canBeModal);
  for (i=0; i<m_slots.count(); i++)
  {
    SlotItem *slitem = m_slots[i];
  ...
  if (m_creatingNewSubclass)
    saveBuffer(buffer,m_formPath + "/" + m_edFileName->text()+".java");
  else
    saveBuffer(buffer,m_filename+".java");

  if (m_creatingNewSubclass)
  {
    m_newFileNames.append(m_formPath + "/" + m_edFileName->text()+".java");
