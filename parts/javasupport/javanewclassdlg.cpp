...
JavaNewClassDialog::JavaNewClassDialog(KDevPlugin *part, QWidget *parent, const char *name)
	: JavaNewClassDialogBase(parent, name)
{
    ...
    interface_url = DomUtil::readEntry(dom, "/javasupportpart/filetemplates/interfaceURL");
    implementation_url = DomUtil::readEntry(dom, "/javasupportpart/filetemplates/implementationURL");
    interface_suffix = DomUtil::readEntry(dom, "/javasupportpart/filetemplates/interfacesuffix", ".h");
    implementation_suffix = DomUtil::readEntry(dom, "/javasupportpart/filetemplates/implementationsuffix", ".java");
    lowercase_filenames = DomUtil::readBoolEntry(dom, "/javasupportpart/filetemplates/lowercasefilenames", true);
    m_parse = DomUtil::readEntry( *m_part->projectDom(), "/javasupportpart/newclass/filenamesetting","none");
...
JavaNewClassDialog::~JavaNewClassDialog()
{
    delete comp;
}

void JavaNewClassDialog::setCompletion(ClassStore *store)
{
...
void JavaNewClassDialog::nameHandlerChanged(const QString &text)
{
	DomUtil::writeEntry( *m_part->projectDom(), "/javasupportpart/newclass/filenamesetting",text);
	m_parse = text;
	classNameChanged(classname_edit->text());
}

void JavaNewClassDialog::classNameChanged(const QString &text)
{
    QString str = text;
...
void JavaNewClassDialog::baseclassname_changed(const QString &text)
{
...
void JavaNewClassDialog::baseIncludeChanged(const QString &text)
{
...
void JavaNewClassDialog::headerChanged()
{
...
void JavaNewClassDialog::implementationChanged()
{
	// Only if a change caused by the user himself
...
void JavaNewClassDialog::checkObjCInheritance(int val)
{
...
void JavaNewClassDialog::checkQWidgetInheritance(int val)
{
    if (val)
    {
        qobject_box->setEnabled(val);
...
/*        constructors_java_edit->append(classname_edit->text() + "::" + classname_edit->text() +
...
void JavaNewClassDialog::qobject_box_stateChanged(int val)
{
...
void JavaNewClassDialog::gtk_box_stateChanged(int val)
{
    class_tabs->setTabEnabled(tab2, !val);
    childclass_box->setEnabled(!val);
    objc_box->setEnabled(!val);
...
void JavaNewClassDialog::accept()
{
...
void JavaNewClassDialog::setStateOfInheritanceEditors(bool state, bool hideList)
{
...
void JavaNewClassDialog::addBaseClass()
{
    baseincludeModified = false;
    if (baseclasses_view->selectedItem())
...
void JavaNewClassDialog::remBaseClass()
{
...
void JavaNewClassDialog::remBaseClassOnly()
{
    if (baseclasses_view->selectedItem())
    {
        QListViewItem *it = baseclasses_view->selectedItem();
...
void JavaNewClassDialog::remClassFromAdv(QString text)
{
    QListViewItem *it = 0;
...
/*        int *para = new int(1);
        int *index = new int(1);
        if (constructors_java_edit->find(text + "(", true, false, true, para, index))
        {
            qWarning("%s( found", text.latin1());
            if (para) constructors_java_edit->removeParagraph(*para);
        }*/
        delete it;
    }
}

void JavaNewClassDialog::currBaseNameChanged(const QString &text)
{
...
void JavaNewClassDialog::currBasePrivateSet()
{
    if ( baseclasses_view->selectedItem() )
    {
...
void JavaNewClassDialog::currBaseProtectedSet()
{
    if ( baseclasses_view->selectedItem() )
    {
...
void JavaNewClassDialog::currBasePublicSet()
{
    if ( baseclasses_view->selectedItem() )
    {
...
void JavaNewClassDialog::scopeboxActivated(int value)
{
    if ( baseclasses_view->selectedItem() )
    {
...
void JavaNewClassDialog::currBaseVirtualChanged(int val)
{
    if ( baseclasses_view->selectedItem() )
    {
...
void JavaNewClassDialog::currBaseSelected(QListViewItem *it)
{
    if (it == 0)
    {
...
void JavaNewClassDialog::upbaseclass_button_clicked()
{
    bool basename_focused = false;
    if (basename_edit->hasFocus())
    {
...
void JavaNewClassDialog::downbaseclass_button_clicked()
{
    bool basename_focused = false;
    if (basename_edit->hasFocus())
    {
...
void JavaNewClassDialog::updateConstructorsOrder()
{
...
void JavaNewClassDialog::checkUpButtonState()
{
    if (baseclasses_view->selectedItem())
        upbaseclass_button->setEnabled(baseclasses_view->selectedItem()->itemAbove());
}

void JavaNewClassDialog::checkDownButtonState()
{
    if (baseclasses_view->selectedItem())
        downbaseclass_button->setEnabled(baseclasses_view->selectedItem()->itemBelow());
}

void JavaNewClassDialog::baseclasses_view_selectionChanged()
{
/*    if (baseclasses_view->selectedItem())
    {
        setStateOfInheritanceEditors(false, false);
...
void JavaNewClassDialog::changeToPrivate()
{
    if (access_view->selectedItem())
        access_view->selectedItem()->setText(2, "private");
}

void JavaNewClassDialog::changeToProtected()
{
    if (access_view->selectedItem())
        access_view->selectedItem()->setText(2, "protected");
}

void JavaNewClassDialog::changeToPublic()
{
    if (access_view->selectedItem())
        access_view->selectedItem()->setText(2, "public");
}

void JavaNewClassDialog::changeToInherited()
{
    if (access_view->selectedItem())
        access_view->selectedItem()->setText(2, QString::null);
}

void JavaNewClassDialog::newTabSelected(const QString& /*text*/)
{
/*    if (text == i18n("&Advanced Inheritance"))
        reloadAdvancedInheritance(true);*/
}

void JavaNewClassDialog::newTabSelected(QWidget* /*w*/)
{
...
void JavaNewClassDialog::reloadAdvancedInheritance(bool /*clean*/)
{
...
void JavaNewClassDialog::parseClass(QString clName, QString inheritance)
{
...
bool JavaNewClassDialog::isConstructor(QString className, ParsedMethod *method)
{
...
void JavaNewClassDialog::addToConstructorsList(QCheckListItem *myClass, ParsedMethod *method)
{
...
void JavaNewClassDialog::addToMethodsList(QListViewItem *parent, ParsedMethod *method)
{
...
void JavaNewClassDialog::addToUpgradeList(QListViewItem *parent, ParsedMethod *method, QString modifier)
{
    PListViewItem<ParsedMethod*> *it = new PListViewItem<ParsedMethod*>(method, parent, method->asString());
    it->setText(1, modifier);
}

void JavaNewClassDialog::addToUpgradeList(QListViewItem *parent, ParsedAttribute *attr, QString modifier)
{
    PListViewItem<ParsedAttribute*> *it = new PListViewItem<ParsedAttribute*>(attr, parent, attr->asString());
    it->setText(1, modifier);
}

void JavaNewClassDialog::clear_selection_button_clicked()
{
...
void JavaNewClassDialog::clearConstructorsList(bool clean)
{
    if (clean)
...
void JavaNewClassDialog::clearMethodsList(bool clean)
{
    if (clean)
        methods_view->clear();
}

void JavaNewClassDialog::clearUpgradeList(bool clean)
{
    if (clean)
        access_view->clear();
}

void JavaNewClassDialog::setAccessForItem(QListViewItem *curr, QString newAccess, bool isPublic)
{
    if (newAccess == "public")
...
void JavaNewClassDialog::setAccessForBase(QString baseclass, QString newAccess)
{
    QListViewItem *base;
...
void JavaNewClassDialog::access_view_mouseButtonPressed( int button, QListViewItem * item, const QPoint &p, int /*c*/ )
{
...
void JavaNewClassDialog::methods_view_mouseButtonPressed(int button ,QListViewItem * item, const QPoint&p ,int /*c*/)
{
...
void JavaNewClassDialog::extendFunctionality()
{
    if (methods_view->selectedItem())
        methods_view->selectedItem()->setText(1, i18n("extend"));
}

void JavaNewClassDialog::replaceFunctionality()
{
    if (methods_view->selectedItem())
        methods_view->selectedItem()->setText(1, i18n("replace"));
}

void JavaNewClassDialog::selectall_button_clicked()
{
    QListViewItemIterator it( constructors_view );
    while ( it.current() )
    {
...
void JavaNewClassDialog::to_constructors_list_clicked()
{
    QString constructor_h = classname_edit->text();
    QString constructor_java = classname_edit->text() + "::" + classname_edit->text();
    constructor_h += "(";
    constructor_java += "(";
    QString params_h;
    QString params_java;
    ...
    constructor_java += params_java + ")" + base + QString("\n{\n}\n\n\n");
    constructor_h += params_h + ");\n\n";

    constructors_h_edit->append(constructor_h);
    constructors_java_edit->append(constructor_java);
}

void JavaNewClassDialog::updateClassStore()
{
}
...
bool JavaNewClassDialog::ClassGenerator::validateInput()
{
...
bool JavaNewClassDialog::ClassGenerator::generate()
{
  if (!validateInput())
    return false;
...
void JavaNewClassDialog::ClassGenerator::common_text()
{
  ...
  if (!dlg.constructors_java_edit->text().isEmpty())
  {
      advConstructorsSource = dlg.constructors_java_edit->text();
  }
  advConstructorsHeader.replace(QRegExp("[\\n ]*$"), QString::null);
...
//        if (advJava.isNull()) advJava += "\n\n";

        genMethodDeclaration(curr->item(), className, adv_h, &advJava,
...
/*    if ((*adv_h).isNull())
  ...
  advJava.replace(e, QString::null);
}

void JavaNewClassDialog::ClassGenerator::genMethodDeclaration(ParsedMethod *method,
    QString className, QString *adv_h, QString *adv_java, bool extend, QString baseClassName )
{
/*    if ((*adv_h).isNull())
        *adv_h += "\n\n";*/
    ...
    *adv_java += method->type() + " " + className + "::" + methodName + "(";
    ...
    *adv_java += cparams + ")" + (method->isConst() ? " const" : "") + "\n{\n";
    if (extend)
        *adv_java += ((method->type() == "void") ? "    " : "    return ") +
                    baseClassName + "::" + methodName + "(" + bparams + ");\n";
    *adv_java += "}\n\n";
}


void JavaNewClassDialog::ClassGenerator::gen_implementation()
{
  ...
  if (dlg.filetemplate_box->isChecked()) {
    ...
    if(DomUtil::readBoolEntry(dom,"/javasupportpart/filetemplates/choosefiles",false))
      classImpl = FileTemplate::read(dlg.m_part, DomUtil::readEntry(dom,"/javasupportpart/filetemplates/implementationURL",""), FileTemplate::Custom);
    else
      classImpl = FileTemplate::read(dlg.m_part, "java");
  }
  if (objc) {
  ...
  } else if (gtk)
  {
    ...
    classImpl += dlg.gen_config->javaSource();
/*    classImpl += QString( 
			 "#include \"$HEADER$\"\n"
			 "\n"
			 "\n")
  ...
  if (childClass)
  {
    ...
    argsJava = "QWidget *parent, const char *name, WFlags f";
  }
  else if (qobject)
  {
    argsH = "QObject *parent = 0, const char *name = 0";
    argsJava = "QObject *parent, const char *name";
  }
  else
  {
    argsH = "";
  ...
  constructors.replace(QRegExp("\\$ARGS\\$"), argsJava);

  
  //remove unnesessary carriadge returns
  QString hp = relPath+header;
  beautifySource(classImpl, hp, className, namespaceBeg, constructors, advJava, namespaceEnd, implementation);
  ...
  classImpl.replace(QRegExp("\\$DEFINITIONS\\$"), advJava);
...
void JavaNewClassDialog::ClassGenerator::gen_interface()
{
  // interface
  ...
  if (dlg.filetemplate_box->isChecked()) {
    QDomDocument dom = *dlg.m_part->projectDom();
    if(DomUtil::readBoolEntry(dom,"/javasupportpart/filetemplates/choosefiles",false))
      classIntf = FileTemplate::read(dlg.m_part, DomUtil::readEntry(dom,"/javasupportpart/filetemplates/interfaceURL",""), FileTemplate::Custom);
    else
  ...
  if (objc) {
    ...
    classIntf += dlg.gen_config->javaHeader();
/*    classIntf = QString("\n"
			"#ifndef $HEADERGUARD$\n"
			"#define $HEADERGUARD$\n"
			"\n"
...
void JavaNewClassDialog::ClassGenerator::beautifyHeader(QString &templ, QString &headerGuard,
...
void JavaNewClassDialog::ClassGenerator::beautifySource(QString &templ, QString &header, QString &className, QString &namespaceBeg,
    QString &constructors, QString &advJava, QString &namespaceEnd, QString &implementation)
{
    if (header.isEmpty())
