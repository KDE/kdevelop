...
{
    readConfig();
    currTemplate = &javaHeaderText;
    template_edit->setText(*currTemplate);
}

ClassGeneratorConfig::ClassGeneratorConfig(QString v_javaHeaderText, QString v_javaSourceText, 
    ...
    : ClassGeneratorConfigBase(parent,name,fl),
    javaHeaderText(v_javaHeaderText), javaSourceText(v_javaSourceText),
...
QString ClassGeneratorConfig::javaHeader()
{
    if (currTemplate == &javaHeaderText)
        *currTemplate = template_edit->text();
    return javaHeaderText;
}

QString ClassGeneratorConfig::javaSource()
{
    if (currTemplate == &javaSourceText)
        *currTemplate = template_edit->text();
...
{
    switch( value ){
        case 0: return &javaHeaderText;
        case 1: return &javaSourceText;
        case 2: return &objcHeaderText;
        case 3: return &objcSourceText;
        case 4: return &gtkHeaderText;
        case 5: return &gtkSourceText;
    }
    return 0;
}
...
void ClassGeneratorConfig::readConfig()
{
    ...
    if (config)
    {
        ...
        javaHeaderText = templateText( dirs->findResource("newclasstemplates", "java_header") );
        javaSourceText = templateText( dirs->findResource("newclasstemplates", "java_source") );
...
void ClassGeneratorConfig::storeConfig()
{
    ...
    if (config)
    {
        ...
        saveTemplateText( dirs->saveLocation("newclasstemplates")+"java_header", javaHeader() );
        saveTemplateText( dirs->saveLocation("newclasstemplates")+"java_source", javaSource() );
