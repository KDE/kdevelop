...
void KWriteView::mousePressEvent(QMouseEvent *e) {
  ...
  if (/*kWrite->rmbMenu && */e->button() == RightButton) {
    ...
    getVConfig(c);
    PointStruc cursor;
    cursor.y = (yPos + e->y())/kWriteDoc->fontHeight;
    (void) kWriteDoc->textWidth(c.flags & KWrite::cfWrapCursor, cursor,xPos-2 + e->x());
    emit kWrite->popupMenu(cursor.y, cursor.x);
#else
  ...
  // But here it is not possible, because in the context menu actions,
  // this view may be destroyed. -Bernd
}
