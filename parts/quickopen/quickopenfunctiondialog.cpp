...
void QuickOpenFunctionDialog::gotoFile( QString name )
{
	...
	if( funcList->count() == 1 ){
		...
		for( FunctionDefinitionList::Iterator it = funcList->begin() ; it!=funcList->end() ; ++it ){
			...
			ArgumentList argList = fmodel->argumentList();
			_argStr = "";
			
			for( ArgumentList::Iterator it = argList.begin() ; it!=argList.end() ; ++it ){
				ArgumentModel *argModel = (*it).data();
				argStr = _argStr.append( " " ).append( argModel->type() ).append(" ").append( argModel->name() ).append(',');
			}
			
			if( argStr.isEmpty() ) argStr = i18n(" No argument" );
			fdlg.argBox->insertItem( argStr );
