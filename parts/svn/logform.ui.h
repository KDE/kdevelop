...
{
	// "svn log" needs to be done on relative-path basis
	pathName = path;
	...
	process->setArguments( QStringList() << "svn" << "log" << pathName );
...
void LogForm::linkClicked( const QString & link )
{
	...
	process->setArguments( QStringList() << "svn" << "diff" << "-r" + v1 << "-r" + v2 << pathName );
	process->start();
