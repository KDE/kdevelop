...
/// NOTE: shared_ptr indirection to get stable location to allow safe recursion
using CachedDeclarationsByImports = QHash<TopDUContext::IndexedRecursiveImports, std::shared_ptr<CachedDeclarations>>;
    ...
    LockedItemRepository::write<PersistentSymbolTable>([&](PersistentSymbolTableRepo& repo) {
        ...
        const auto cachedImports = [&]() {
            ...
            if (it == repo.importsCache.constEnd()) {
                ...
                it = repo.importsCache.insert(visibility, std::move(cachedImports));
            }
            return *it;
        }();
        ...
        auto filterIterator = [&]() {
            ...
            if (cacheIt == cached.constEnd()) {
                auto cache = std::make_shared<CachedDeclarations>();
                {
                ...
                cacheIt = cached.insert(visibility, std::move(cache));
            }

            // NOTE: safe reference even during recursion due to shared_ptr indirection
            const auto& cache = *cacheIt->get();
