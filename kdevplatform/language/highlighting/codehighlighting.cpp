...
void CodeHighlighting::applyHighlighting(void* _highlighting)
{
    ...
    while (rangeIt != highlighting->m_waiting.end()) {
        ...
        if (movingIt == oldHighlightedRanges.end() ||
            ...
            const auto& range =
                highlighting->m_highlightedRanges.emplace_back(tracker->document()->newMovingRange(tempRange));
            range->setAttribute(rangeIt->attribute);
            range->setZDepth(highlightingZDepth);
        } else
        {
