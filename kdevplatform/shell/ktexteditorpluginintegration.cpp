...
/**
 * @return an IDocument that wraps a given KTextEditor::Document or nullptr if no matching IDocument
 */
...
public:
        ...
        : OutputJob(nullptr, Silent)
    {
        ...
        setToolIcon(QIcon::fromTheme(QStringLiteral("system-run")));
...
{
    if (KTextEditor::Editor::instance()->application()->openUrl(url, encoding)) {
...
{
    return new KTextEditorConfigPageAdapter(m_plugin->configPage(number, parent));
}
