...
{
    if (auto iface = qobject_cast<KTextEditor::Document*>(part))
    {
        iface->setMarkDescription((KTextEditor::Document::MarkTypes) BreakpointMark, i18n("Breakpoint"));
        iface->setMarkIcon((KTextEditor::Document::MarkTypes) BreakpointMark, *breakpointPixmap());
        iface->setMarkIcon((KTextEditor::Document::MarkTypes) PendingBreakpointMark, *pendingBreakpointPixmap());
        iface->setMarkIcon((KTextEditor::Document::MarkTypes) ReachedBreakpointMark, *reachedBreakpointPixmap());
        iface->setMarkIcon((KTextEditor::Document::MarkTypes) DisabledBreakpointMark, *disabledBreakpointPixmap());
        iface->setEditableMarks( KTextEditor::Document::Bookmark | BreakpointMark );
        updateMarks();
    }
}
...
void KDevelop::BreakpointModel::updateMarks()
{
    ...
    for (Breakpoint* breakpoint : qAsConst(d->breakpoints)) {
        ...
        if (!doc) continue;
        KTextEditor::Document *mark = doc->textDocument();
        if (!mark) continue;
...
void BreakpointModel::setupMovingCursor(KTextEditor::Document* document, Breakpoint* breakpoint) const
{
    ...
    if (document) {
        auto* const cursor = document->newMovingCursor(KTextEditor::Cursor(breakpoint->line(), 0));
