...
void BreakpointModel::removeBreakpointMarks(const KTextEditor::Document& document)
{
    ...
    // and performance (the QList is smaller than the QHash and is ideal for the loop above).
    const auto markLines = marks.keys();
    for (int line : markLines) {
...
void BreakpointModel::aboutToInvalidateMovingInterfaceContent(KTextEditor::Document* document)
{
    ...
    d->reloadState = hasBreakpointMarks && document->isModified() ? ReloadState::RemoveBreakpointMarksAndReinitialize
