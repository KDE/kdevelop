...
void Breakpoint::stopDocumentLineTracking()
{
    ...
    auto* const doc = m_movingCursor->document();
    doc->removeMark(m_movingCursor->line(), BreakpointModel::AllBreakpointMarks);
...
void Breakpoint::restartDocumentLineTrackingAt(KTextEditor::MovingCursor* cursor)
{
    ...
    auto* const doc = m_movingCursor->document();
    const auto guard = m_model->markChangeGuard();
    doc->addMark(m_movingCursor->line(), markType());
}
...
void Breakpoint::updateMarkType() const
{
    ...
    const auto oldMarkType = doc->mark(docLine) & BreakpointModel::AllBreakpointMarks;
    ...
    const auto guard = m_model->markChangeGuard();
    doc->removeMark(docLine, BreakpointModel::AllBreakpointMarks);
    doc->addMark(docLine, newMarkType);
}
